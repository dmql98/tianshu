...
export async function executeTool(name: string, args: ToolArgs, workspace: string, signal?: AbortSignal, mcpClients?: Map<string, MCPClient>, allowedRoots?: string[], onOutput?: (chunk: string) => void,     workspaces?: string[], sessionId?: string, characterId?: string, knowledgeBases?: string[]): Promise<ToolResult> {
  ...
  try {
    ...
    return await registryExecute(name, args, { sessionId, characterId, workspace, workspaces: mergedWorkspaces, signal, allowedRoots, onOutput, knowledgeBases })
  } catch (err: any) {
