...
for (const r of db.prepare('SELECT id,status,phase,queued_at,started_at,finished_at,resumed_from_run_id FROM runs WHERE session_id=? ORDER BY queued_at').all(sid)) {
  console.log(JSON.stringify({ ...r, queued: r.queued_at ? new Date(r.queued_at).toLocaleTimeString('zh-CN') : null, started: r.started_at ? new Date(r.started_at).toLocaleTimeString('zh-CN') : null, finished: r.finished_at ? new Date(r.finished_at).toLocaleTimeString('zh-CN') : null }))
}
...
const rows = db.prepare("SELECT run_id,type,created_at,substr(payload,1,150) p FROM run_events WHERE session_id=? AND type NOT IN ('message.delta','tool.output') ORDER BY created_at").all(sid)
for (const e of rows) {
...
console.log('total non-delta:', rows.length)
