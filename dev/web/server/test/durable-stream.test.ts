...
/** NOOP shim：与 ws/handlers.ts 的 NOOP_STREAM 语义一致（delivery via sinks）。 */
...
describe('non-durable stream events still reach sinks (R8 regression)', () => {
  it('message.delta emitted through the durable stream fans out to sinks', () => {
    clearEventSinks()
    ...
    // 模拟 loop 路径：inner.ts 持有 durable stream 并 emit message.delta。
