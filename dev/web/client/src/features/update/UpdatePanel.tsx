  ...
  const renderControls = () => {
    switch (state.phase) {
      ...
      case 'available':
        return (
          <div className="setting-row">
            <div className="setting-info"><span className="setting-label">发现新版本</span><span className="setting-hint">发现 {state.targetVersion ? `v${state.targetVersion}` : '新版本'}，点击下载后手动安装</span></div>
      ...
      case 'downloading': {
        ...
        return (
          <div className="setting-row" style={{ alignItems: 'flex-start' }}>
            <div className="setting-info"><span className="setting-label">正在下载</span></div>
      ...
      case 'downloaded':
        ...
        return (
          <div className="setting-row">
            <div className="setting-info"><span className="setting-label">更新已就绪</span><span className="setting-hint">安装时将自动重启天枢</span></div>
            <div className="setting-control">
              <button className="btn primary" style={{ marginRight: 8 }} onClick={() => void install()}>立即重启安装</button>
      ...
      default:
        return (
          <div className="setting-row">
            <div className="setting-info"><span className="setting-label">检查更新</span>
              <span className="setting-hint">
                {state.phase === 'not-available' ? `已是最新版本（${state.checkedAt ? new Date(state.checkedAt).toLocaleString() : ''}）` : '手动检查 GitHub Releases 上的新版本'}
              </span>
            </div>
