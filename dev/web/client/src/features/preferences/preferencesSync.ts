/**
 * 偏好服务端同步（USER_PREFERENCES_PLAN）。
 *
 * 把主题/图标包选择同步到服务端 <dataDir>/user-preferences.json，让它们跨重启
 * 保留（桌面客户端每次启动随机端口 → localStorage origin 变化，不能作为持久层）。
 *
 * 流程：
 * - 启动时 `syncPreferencesFromServer()`：拉取服务端偏好并应用（存在才覆盖，
 *   服务端无数据时保留本地值），随后才订阅变更推送，避免自写回环。
 * - 运行期订阅 `tianshu:theme-changed` / `tianshu:iconpack-changed`，把用户
 *   选择异步推到服务端（串行队列 + 尾写优先，不阻塞 UI）。
 */
...
/** 启动时应用服务端偏好；服务端无对应字段时不动本地值。 */
export async function syncPreferencesFromServer(): Promise<void> {
  const prefs = await fetchUserPreferences()
  if (prefs.theme) {
    setThemeSelection(loadThemePreferences(), prefs.theme)
  }
  if (prefs.iconPack?.packId) {
    setActiveIconPack(prefs.iconPack.packId)
  }
}
...
export function persistPreferenceChanges(): () => void {
  const onThemeChanged = (event: Event): void => {
    ...
    if (selection) enqueuePush(() => updateUserPreferences({ theme: selection }))
  }
  const onIconPackChanged = (event: Event): void => {
    const packId = (event as CustomEvent<{ packId?: string }>).detail?.packId
    if (packId) enqueuePush(() => updateUserPreferences({ iconPack: { packId } }))
  }
